...
POETRIES = [
    ...
    (5, '凶', '家道未能昌\\n危危保禍殃\\n暗雲侵月桂\\n佳人一炷香', '就算試著努力家業，和努力相比卻難以繁盛起來吧。不是一生災禍，只是危險的事比較多而已吧。像烏雲遮月一樣，一生阻礙比較多吧。在身份高貴的婦人房裡，各種想法像香一樣擴散開來般，心裡無法平靜吧。願望：難以實現吧。疾病：難治好吧。遺失物：難找到吧。盼望的人：不會出現吧。蓋新居、搬家：先放棄，暫時觀察情況再說吧。結親緣、旅行、交往：因為萬事凶惡，請諸行為慎重行事。'),
    ...
    (8, '大吉', '勿頭中見尾\\n文華須得理\\n禾刀自偶然\\n當遇非常喜', '在腦海裡沒有對失敗或結果的恐懼，抬頭挺胸地朝著目標達成而努力吧。無論是文學或武術都能得到真理，要有充實自己的心態吧。就像用刀來割稻般地，會得到成果，幸福會自然地到來吧。如果正心地守道的話，可以變得幸福吧。願望：會實現吧。疾病：會治好。請注意養生吧。遺失物：可以找到吧。盼望的人：會出現吧。蓋新居、搬家、交往：是好事吧。旅行：途中請不要粗心大意吧。結親緣：全都是好的吧。'),
    (9, '大吉', '有名須得遇\\n三望一朝遷\\n貴人來指處\\n華果應時鮮', '按照所想的，夢想實現，名聲也廣傳人間吧。就像三個願望可以完全的實現般地，全部能一次完全的被實現吧。從比自己身份地位高的人，給予各式各樣令人欣喜的事吧。像是隨著四季循環花會開、會結果一樣，每天努力會有成果，好運會展開吧。願望：會實現吧。疾病：會治好吧。遺失物：會找到吧。盼望的人：會出現吧。蓋新居、搬家：好吧。結親緣、交往：全都好吧。旅行：沒問題吧。'),
    ...
    (12, '大吉', '楊柳遇春時\\n殘花發陽枝\\n重重霜雪裡\\n黃金色更輝', '像柳樹也逢春增添綠色般地，希望也會出現吧。像老枝也發芽，開花般地，喜事會到來吧。像在重重霜或雪中般地，無論怎樣過去的勞苦不斷吧，但是像黃金色無論到何時，都閃亮耀眼般地，不要忘以前的勞苦，用誠實的心過生活吧。願望：會實現吧。疾病：會變好吧。遺失物：會回來吧。盼望的人：晚出現吧。蓋新居、搬家：會成為好結果吧。旅行：沒有阻礙吧。結婚、交往：全都適當吧。'), (13, '大吉', '手把大陽輝\\n東君發舊枝\\n稼苗方欲秀\\n猶更上雲梯', '如果時機來到的話，抱著勇氣努力做事，可以過著充實的生活吧。就像是老枝條，隨著春天到來，花就開了一樣，運勢會開展吧。就像稻苗也是如果春天來了的話，就會生長般地，你也會繁榮昌盛吧。像是也能爬上難登的雲梯般，因為更加地持續努力就會成功吧。願望：會實現吧。疾病：會治好吧。遺失物：立刻找就能找到吧。盼望的人：會出現吧。蓋新居、搬家：好吧。旅行：在春天和夏天好吧。結婚、交往：全部都好吧。'), (14, '末吉', '玉石未分時\\n憂心轉更悲\\n前途通大道\\n花發應殘枝', '就像是就算有寶玉，哪一個是寶石哪一個是石頭還分不清楚般地，沒辦法分辨事物的狀況。就算是想要出人頭地，但會為了各式各樣的事情而心痛、嘆息、悲傷也說不定。如果忍耐勞苦，將來自然地看得到未來的去向吧。然後像枯枝開花般地，願望會實現吧。願望：很花時間但會實現吧。疾病：會拖長吧，但是不會影響性命吧。遺失物：難出現吧。盼望的人：似乎會變得遲吧。蓋新居、搬家：不太好吧。結婚：現在要節制，如果往後的話好吧。旅行、交往：避開吧。'),
...
def gen_image(item):
    client_id = '22b769676009ff0'
    client_secret = '5e1c7cc363713d5b2767d549b0fa44953d7e7ca4'
    client = ImgurClient(client_id, client_secret)

    with open('poem@2x.png', 'r') as f:
        surface = cairo.ImageSurface.create_from_png(f)

    cr = cairo.Context(surface)
    cr.set_source_rgb(0, 0, 0)
    cr.select_font_face('華康特粗楷體')

    cr.set_font_size(216)
    cr.move_to(340 - len(unicode(item[1], 'utf8')) / 2.0 * 216, 280)
    cr.show_text(item[1])

    cr.set_font_size(80)
    for i, sentence in enumerate(item[2].split('\\n')):
        for j, c in enumerate(unicode(sentence, 'utf8')):
            cr.move_to(510 - i * 140, 485 + j * 100)
            cr.show_text(c.encode('utf8'))

    num_txt = '第%s番' % number_to_chinese(item[0])
    cr.set_font_size(56)
    cr.move_to(340 - (len(unicode(num_txt, 'utf8')) / 2.0 * 56), 1035)
    cr.show_text(num_txt)

    filename = 'images/%d.png' % item[0]
    surface.write_to_png(filename)
    ret = client.upload_from_path(filename)
    return str(ret['link'])
