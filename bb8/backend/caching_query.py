...
This file is taken from sqlalchemy example git version
@d71f4b47186972c5248c94ee2d04364da95a0965 and modified to pass bb8 pylint.
...
class CachingQuery(Query):
    ...
    def _get_cache_plus_key(self):
        ...
        dogpile_region = self.cache_regions[self.cache_region.region]
        if self.cache_region.cache_key:
            key = self.cache_region.cache_key
        else:
...
def _key_from_query(query, unused_qualifier=None):
...
class RelationshipCache(MapperOption):
    ...
    def process_query_conditionally(self, query):
        ...
        """
        if query.current_path:
            mapper, prop = query.current_path[-2:]
            key = prop.key

            for cls in mapper.class_.__mro__:
                if (cls, key) in self.relationship_options:
                    relationship_option = self.relationship_options[
                        (cls, key)]
                    query.cache_region = relationship_option
                    break
    ...
    def and_(self, option):
        ...
        """
        self.relationship_options.update(option.relationship_options)
        return self
